        ...
        if (isEditing) {
            Object.entries(editedValues).forEach(([accountId, changes]) => {
                if (Object.keys(changes).length > 0) {
                    ...
                    if(changes.name && changes.name.trim() && changes.name !== originalAccount.name) {
                        updatedData.name = changes.name.trim();
                    }
                    if(changes.balance !== undefined && changes.balance !== originalAccount.balance) {
                        updatedData.balance = changes.balance;
                    }

                    if(Object.keys(updatedData).length > 0) {
                       updateAccount({ id: accountId, data: updatedData });
                    }
                }
            });
            setEditedValues({});
    ...
    if (accounts.length === 0) {
        return (
            <EmptyState
                ...
                message="You haven't added any financial accounts yet. Use the AI Agent to add bank accounts, brokerages, or other assets."
                action={
                    <Link to="/ai" className="bg-accent text-white font-semibold py-2 px-4 rounded-lg hover:opacity-90 transition-opacity">
                        Add an Account via AI
                    </Link>
                }
            />
        );
            ...
            <div className="h-48">
                ...
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    <div className="flex items-center justify-center h-full text-text-secondary">
                        <p>No asset data for allocation chart.</p>
                    </div>
                )}
            </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-2 gap-4 sm:gap-0">
            ...
            <button
                ...
                className="flex w-full sm:w-auto justify-center items-center gap-2 bg-secondary hover:bg-primary text-text-primary font-semibold py-2 px-4 rounded-lg transition-colors"
            >
        ...
        {Object.keys(accountsByCategory).length > 0 ? Object.entries(accountsByCategory).map(([category, accts]) => {
            ...
            <Card>
                <p className="text-center text-text-secondary py-8">No accounts found. Add accounts using the AI agent.</p>
            </Card>
        )}
      </div>
    </div>
