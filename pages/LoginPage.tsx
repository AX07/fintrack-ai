...
const LoginPage: React.FC = () => {
    ...
    const onScanSuccess = useCallback(async (decodedText: string) => {
        if (scannerRef.current && scannerRef.current.isScanning) {
            try {
                ...
                console.warn("Ignoring error while stopping scanner", e);
            }
        }
        setScanError(null);
        ...
        // Attempt 1: Try parsing as new compressed format
        if (trimmedText.startsWith('FINT_C_V1:')) {
            try {
                ...
                console.error("Failed to parse compressed QR code", e);
            }
        }

        // Attempt 2: Try parsing as old uncompressed JSON format
        if (!payload && (trimmedText.startsWith('{') || trimmedText.startsWith('['))) {
            try {
                payload = JSON.parse(trimmedText);
            } catch (e) {
                console.error("Failed to parse uncompressed QR code", e);
            }
        }

        // Process the payload if found, otherwise show error
        if (payload && payload.user && payload.financeData && 'apiKey' in payload) {
            ...
            // Check for old PeerJS UUID format for a more specific error
            const peerJsUuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
            let errorMessage;
            if (peerJsUuidRegex.test(trimmedText)) {
                errorMessage = "This QR code is for an old, unsupported device sync method. Please use the 'New Device Setup' feature on an up-to-date app to generate a valid code.";
            } else {
                errorMessage = "Could not read the QR code. It may be invalid or from an unsupported old version. Please generate a new code from an up-to-date device and try again.";
            }
            console.error("QR Scan Parse Error:", errorMessage, "Raw Text:", decodedText);
    ...
    useEffect(() => {
        if (mode === 'scan') {
            ...
            scanner.start({ facingMode: "environment" }, config, onScanSuccess, (errorMessage) => { /* ignore verbose errors */ })
            .catch(err => {
                ...
                {mode === 'create' ? (
                    <Card>
                        ...
                        <p className="text-sm text-text-secondary text-center mb-6">
                            Track your assets and spending with the help of an AI agent. 
                            FinTrack AI runs entirely on your device, ensuring your financial data is never sent to a server. 
                            Enter a nickname and your Gemini API key to begin.
                        </p>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    id="apiKey" type="password" required value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)}
                                    className="w-full bg-primary border border-secondary rounded-lg px-4 py-2 text-text-primary focus:outline-none focus:ring-2 focus:ring-accent"
                            ...
                            <button
                                type="submit" disabled={!name.trim() || !apiKeyInput.trim()}
                        ...
                        <button
                            type="button" onClick={() => setMode('scan')}
                    ...
                    <Card>
                        ...
                        <p className="text-sm text-text-secondary text-center mb-6">
                            Scan the code from another device to clone its profile.
                        </p>
